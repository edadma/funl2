    ...
    Map(
      ...
      "array" -> { (_: VM, apos: Position, ps: List[Position], args: Any) =>
        argsderef(args) match {
          ...
          case init: Seq[_]
              if init.nonEmpty && init.head.isInstanceOf[Seq[Any]] =>
            mutable.ArraySeq[Any](init.asInstanceOf[Seq[Seq[Any]]] map (e =>
              mutable.ArraySeq[Any](e: _*)): _*)
      ...
      "buffer" -> { (_: VM, apos: Position, ps: List[Position], args: Any) =>
        argsderef(args) match {
          ...
          case init: Seq[_]
              if init.nonEmpty && init.head.isInstanceOf[Seq[Any]] =>
            ArrayBuffer[Any](init map (e =>
              ArrayBuffer[Any](e.asInstanceOf[Seq[Any]]: _*)): _*)
      ...
      "seq" -> { (_: VM, apos: Position, ps: List[Position], args: Any) =>
        argsderef(args) match {
          ...
          case init: Seq[_]
              if init.nonEmpty && init.head.isInstanceOf[Seq[Any]] =>
            immutable.ArraySeq[Any](init map (e =>
              immutable.ArraySeq[Any](e.asInstanceOf[Seq[Any]]: _*)): _*)
      ...
      "table" -> { (_: VM, apos: Position, ps: List[Position], args: Any) =>
        argsderef(args) match {
          ...
          case s: Iterable[_] if s.head.isInstanceOf[Tuple] =>
            LinkedHashMap(s.asInstanceOf[Iterable[Tuple]].toSeq map (v =>
              (v.element(0), v.element(1))): _*)
        }
      },
      ...
      "center" -> { (vm: VM, apos: Position, ps: List[Position], args: Any) =>
        val (src, width, padding) =
          argsderef(args) match {
            case ArgList() =>
              problem(ps.headOption.getOrElse(apos),
                      "center( src, width, padding )")
            case s: String                   => (s, 1, " ")
      ...
      "chr" -> { (vm: VM, apos: Position, ps: List[Position], args: Any) =>
        deref(args) match {
          ...
          case x =>
            problem(ps.headOption.getOrElse(apos),
                    s"chr: expected a (small) integer")
        }
      },
      "ord" -> { (vm: VM, apos: Position, ps: List[Position], args: Any) =>
        deref(args) match {
          ...
          case x =>
            problem(ps.headOption.getOrElse(apos),
                    s"ord: expected a non-empty string")
        }
      },
      ...
      "sleep" -> { (vm: VM, apos: Position, ps: List[Position], args: Any) =>
        deref(args) match {
          ...
          case x =>
            problem(ps.headOption.getOrElse(apos),
                    s"sleep: expected a non-negative integer (milliseconds)")
        }
      }
...
//			case s: String => s.toDouble
  ...
  private lazy val devrandom = new DataInputStream(
    new FileInputStream("/dev/random"))
